...
bool BattleScene::init(rapidjson::Value datas){
    if (!CCLayer::init()) {
    ...
    money=datas["coin"].GetInt();
    mxp=datas["xp"].GetInt();
    card=datas["card"].GetInt();
    zhuangbei=datas["sword"].GetInt();
...
void BattleScene::getJsonData(const char *data){
    ...
    jsonData.Parse<0>(data);
    if (jsonData.HasParseError()) {
        log("GetParseError%d", jsonData.GetParseError());
    }
    alluseValue = jsonData["data"];
}
void BattleScene::initAttackData(rapidjson::Value datas)
{
    ...
    CCSize winSize= CCDirector::sharedDirector()->getWinSize();
    int level=datas["chalevel"].GetInt();
    for (int i=0; i<6; i++)
    {
        char str[32];
        sprintf(str,"chaPos%d",i+1);
        int type=datas["startData"][str].GetInt();
        type-=200;
